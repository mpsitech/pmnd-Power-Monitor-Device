/**
	* \file Pmnd.cpp
	* Pmnd global functionality and unit/controller exchange (implementation)
...
UntPmnd::UntPmnd()
		:
			mAccess("mAccess", "UntPmnd", "UntPmnd")
			, rwmHist("rwmHist", "UntPmnd", "UntPmnd")
		{
	initdone = false;;
...
UntPmnd::~UntPmnd() {
	...
	mAccess.lock("UntPmnd", "~UntPmnd");
	mAccess.unlock("UntPmnd", "~UntPmnd");
};

void UntPmnd::lockAccess(
			const string& who
		) {
	mAccess.lock(who);
};

void UntPmnd::unlockAccess(
			const string& who
		) {
	mAccess.unlock(who);
};
...
uint8_t UntPmnd::getTixVControllerBySref(
			const string& sref
		) {
	return 0;
};

string UntPmnd::getSrefByTixVController(
			const uint8_t tixVController
		) {
	return("");
};

void UntPmnd::fillFeedFController(
			Feed& feed
		) {
};

uint8_t UntPmnd::getTixWBufferBySref(
			const string& sref
		) {
	return 0;
};

string UntPmnd::getSrefByTixWBuffer(
			const uint8_t tixWBuffer
		) {
	return("");
};

void UntPmnd::fillFeedFBuffer(
			Feed& feed
		) {
};

uint8_t UntPmnd::getTixVCommandBySref(
			const uint8_t tixVController
			, const string& sref
		) {
	return 0;
...
string UntPmnd::getSrefByTixVCommand(
			const uint8_t tixVController
			, const uint8_t tixVCommand
		) {
	return("");
...
void UntPmnd::fillFeedFCommand(
			const uint8_t tixVController
			, Feed& feed
		) {
};

Bufxf* UntPmnd::getNewBufxf(
			const uint8_t tixWBuffer
			, const size_t reqlen
		) {
	return NULL;
...
void UntPmnd::clearHist() {
	rwmHist.wlock("UntPmnd", "clearHist");

	hist.clear();

	rwmHist.wunlock("UntPmnd", "clearHist");
};

void UntPmnd::appendToHist(
			const string& s
		) {
	rwmHist.wlock("UntPmnd", "appendToHist");

	while (hist.size() > histlimit) hist.pop_front();
	hist.push_back(s);

	rwmHist.wunlock("UntPmnd", "appendToHist");
};

void UntPmnd::appendToLastInHist(
			const string& s
		) {
	rwmHist.wlock("UntPmnd", "appendToLastInHist");

	if (!hist.empty()) hist.back() += s;

	rwmHist.wunlock("UntPmnd", "appendToLastInHist");
};

void UntPmnd::copyHist(
			vector<string>& vec
			, const bool append
		) {
	rwmHist.rlock("UntPmnd", "copyHist");

	if (!append) vec.clear();
	for (auto it = hist.begin(); it != hist.end(); it++) vec.push_back(*it);

	rwmHist.runlock("UntPmnd", "copyHist");
};
